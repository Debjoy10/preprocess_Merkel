import sys, time, os, tqdm, torch, argparse, glob, subprocess, warnings, cv2, pickle, numpy, pdb, math, python_speech_features, shutil
import tempfile
...
parser.add_argument('--output_dir', type=str, default='/raid/dsaha/Merkel_TTS_Dataset/talknet_scores', help='Store Talknet Files here')
...
with open(args.videolist, 'r') as f:
    allvideoscenes = [fi.strip() for fi in f.readlines()]
...
def main():
    ...
    savePathparent = args.output_dir
    current_Files = os.listdir(savePathparent)
    badvideos = []
    
    # Changed - #2
    for video in tqdm.tqdm(allvideoscenes):
        # try:
        # Changed - #3
        videoname = video.split('/')[-2] + '_and_' + video.split('/')[-1].split('.')[0]
        if videoname in current_Files:
            print("Skipping {}. Already done".format(videoname))
            continue
            
        args.videoPath = video
        args.savePath = os.path.join(savePathparent, videoname)
        tmpsavePath = tempfile.mkdtemp()

        # Initialization 
        args.pyaviPath = os.path.join(tmpsavePath, 'pyavi')
        args.pyframesPath = os.path.join(tmpsavePath, 'pyframes')
        args.pyworkPath = os.path.join(args.savePath, 'pywork')
        args.pycropPath = os.path.join(tmpsavePath, 'pycrop')
    ...
    #             visualization(vidTracks, scores, args)	
        ...
        #     print(e)
        #     videoname = video.split('/')[-2].strip('.avi')
        #     print("Skipping {}. Error".format(videoname))
        #     badvideos.append(video)
    
    print("All VIDEOS WITH ERROR ==>")
    print(badvideos)
